...
app.post("/api/join", async (req, res) => {
  try {
    ...
    // Génération du certificat si inscription en ligne
    if (inscription_mode === "online") {
      ...
      // Nom et type
      page.drawText(`Nom: ${name}`, { x: 50, y: 360, size: 16 });
      if (member_type === "organization")
        page.drawText(`Organisation: ${org_name}`, { x: 50, y: 330, size: 16 });
    ...
    // Envoi mail avec ou sans certificat
    try {
      await sendMail({
