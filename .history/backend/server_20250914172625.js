...
app.use("/certificat", express.static(path.join(__dirname, "certificates")));

const { generateCertificate } = require("./generateCertificate");
const fs = require("fs");
const path = require("path");

app.post("/api/adhesion-cert", async (req, res) => {
  try {
    ...
    // Génération du PDF
    const pdfBuffer = await generateCertificate({ name, type });

    // Envoi par email
    await sendMail({
      subject: "Votre certificat d'adhésion UPC2025",
      html: `<p>Bonjour ${name},</p><p>Veuillez trouver votre certificat ci-joint.</p>`,
      attachments: [
        {
          filename: "certificat-adhesion.pdf",
          content: pdfBuffer,
        },
      ],
      to: email,
    ...
    // Réponse avec possibilité de téléchargement direct
    const filePath = path.join(__dirname, "temp", `${name}-certificat.pdf`);
    fs.writeFileSync(filePath, pdfBuffer);
    res.json({ ok: true, downloadUrl: `/download/${name}-certificat.pdf` });
  } catch (err) {
