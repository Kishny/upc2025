...
// Gestion du formulaire adhésion
...
form.addEventListener("submit", async (e) => {
  ...
  const res = await postForm("/api/join", data);
  if (!res.ok) return alert("Erreur : " + (res.error || "Serveur"));

  if (mode === "online" && res.pdfPath) {
    alert("Formulaire envoyé ! Téléchargement du certificat…");
    window.open(`${BASE_URL}/${res.pdfPath}`, "_blank");
  } else {
    alert(
...
form.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    if (mode === "online") {
      ...
      window.location.href = `${BASE_URL}/certificat/${result.id}.pdf`;
    } else {
      alert(
        "Formulaire envoyé ! Vous pouvez maintenant télécharger la fiche papier."
      );
