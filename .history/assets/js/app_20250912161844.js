...
if (video && image) {
  // Lecture / fin vidéo → image → relance vidéo
  video.addEventListener("ended", () => {
    image.style.opacity = 1; // montre l'image
    setTimeout(() => {
      image.style.opacity = 0; // cache l'image
      video.currentTime = 0; // reset vidéo
      video.play(); // relance la vidéo
    }, pauseTime);
  });

  // Lancer la vidéo dès que possible
  video.play().catch(() => {
    console.log("Autoplay bloqué, vérifier si muted est activé");
  });
