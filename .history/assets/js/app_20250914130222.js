...
/* =========================
   ADHÉSION COALITION
========================= */
// Toggle formulaire Organisation / Individuel
document.querySelectorAll('input[name="adhesionType"]').forEach((radio) => {
  radio.addEventListener("change", function () {
    const orgFields = document.getElementById("organization-fields");
    const formWrapper = document.getElementById("form-adhesion");
    if (this.value === "organization" || this.value === "paper") {
      orgFields.style.display = "block";
      formWrapper.classList.add("form-org");
      formWrapper.classList.remove("form-individual");
    } else {
      orgFields.style.display = "none";
      formWrapper.classList.add("form-individual");
      formWrapper.classList.remove("form-org");
    }

    // Si choix papier, rediriger vers page documents
    if (this.value === "paper") {
...
// Form submission -> trigger backend et génération certificat
...
form.addEventListener("submit", async (e) => {
  ...
  try {
    // ⚡ POST vers /api/join
    const res = await fetch("http://127.0.0.1:4000/api/join", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    if (data.ok) {
      alertBox.textContent =
        "Inscription réussie ! Téléchargez votre certificat ci-dessous.";

      // Génération fictive certificat PDF (à remplacer par backend réel)
      const blob = new Blob(["Certificat d'adhésion - UPC 2025"], {
        type: "application/pdf",
      });
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `certificat_${formData.fullname || "membre"}.pdf`;
      a.textContent = "Télécharger le certificat";
      ...
      alertBox.style.borderColor = "#CE1126";
      alertBox.textContent = `Erreur: ${data.error || "Serveur"}`;
    }
  } catch (err) {
    console.error(err);
    alertBox.style.borderColor = "#CE1126";
