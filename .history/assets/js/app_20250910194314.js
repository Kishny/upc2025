...
// Forms (backend): fake success + basic validation
const API_BASE = "http://localhost:4000"; // change si hébergé ailleurs
...
function wireJoinForm() {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      const res = await postJSON(`${API_BASE}/api/join`, {
        name: data.name?.trim(),
      ...
      if (res.ok) {
        ...
        alertBox.textContent =
          "Merci ! Votre adhésion est enregistrée et transmise.";
        form.reset();
      } else {
...
function wireCommitteeForm() {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    const data = Object.fromEntries(new FormData(form));
    try {
      const res = await postJSON(`${API_BASE}/api/committee`, {
      ...
      if (res.ok) {
        ...
        alertBox.style.borderColor = "#007A5E";
        alertBox.textContent =
          "Comité enregistré. Nous vous recontactons rapidement.";
        form.reset();
      } else {
        throw new Error(res.error || "Erreur");
      }
